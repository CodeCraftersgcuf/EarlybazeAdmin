...
const AddWalletSelectable = ({ options, heading, selectedValue, handleOptionSelect, style }) => {
                ...
                <div className="absolute left-0 mt-2 w-full bg-[#042619] border border-gray-500 rounded-lg shadow-lg">
                    <ul className="py-2">
                        {options.map((option, index) => (
                            <li
                                key={index}
                                className={`flex items-center gap-3 px-4 py-2 cursor-pointer ${selectedValue === option.value ? 'text-green-400' : 'text-white'}`}
                                onClick={() => handleOption(option)}
                            >
                                <input type="radio" checked={selectedValue === option.value} readOnly className="mr-2" />
                                {option.name}
                            </li>
                        ))}
                    </ul>
