...
const TransactionModal = ({ status, statusName, options = [], accounts = [], onClose, children }) => {
  const icons = {
    ...
    <Overlay>
      <div className='absolute top-10 right-10 bg-theme-dark p-8 shadow-lg shadow-gray-950 rounded-xl min-w-[35%] max-w-md mx-auto'>
        ...
        <div className="flex flex-col items-center">
          ...
          <p className={`text-${status === 'successful' ? 'green-500' : status === 'processing' ? 'yellow-500' : 'red-500'} font-semibold mt-2`}>
            {statusName}
          </p>
        </div>
          ...
          {accounts && (
            <div className="flex flex-col gap-2">
              <div className='flex items-center justify-between'>
                ...
                <span className='flex items-center gap-2 cursor-pointer' onClick={() => setSelectedAccount(!selectedAccount)}>
                  {accounts.account}<i class={`bi bi-chevron-${selectedAccount ? 'down' : 'right'}`}></i>
                </span>
              </div>
