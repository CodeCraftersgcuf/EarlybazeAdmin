...
const RuleRow = ({ displayData, index }) => {
    ...
    return (
        <>
            <tr className={`hover:bg-green-800 hover:cursor-pointer ${index % 2 === 0 ? "bg-green-950" : ""}`}>
                ...
                <td className='px-4 py-2 text-white text-nowrap'>{displayData.timestamp} / {displayData.time}</td>
                <td className='px-4 py-2'>
                    <div className={`w-1 h-8 rounded-full ${displayData.status === 'active' ? 'bg-green-500' : 'bg-red-500'}`}></div>
                </td>
                <td className='px-4 py-2'>
                    {displayData.exemption && (
                        <button onClick={()=>setShowModel(true)} className='bg-[#25AE7A] py-2 px-4 rounded-lg text-white font-bold'>
                            Exemptions
                        </button>
                    )}
                </td>
            ...
            {showModel && (<SelectExemptionModal
