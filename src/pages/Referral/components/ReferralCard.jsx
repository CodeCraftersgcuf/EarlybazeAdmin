...
const ReferralCard = ({ icon, title,subTitle, value, subText1, subText2, value1, value2 }) => {
    return (
        <div className="bg-[#072117] p-6 rounded-lg shadow-md border border-[#25ae7932] min-w-[250px]">
            <div className="flex items-center gap-4 mb-4">
                {icon && <div className='flex items-center justify-center p-2 bg-white rounded-full'><img src={icon} alt="icon" className="w-8 h-8 rounded-full" /></div>}
                <div>
                    <h3 className={`${value && 'opacity-50'} text-md font-semibold`}>{title}</h3>
