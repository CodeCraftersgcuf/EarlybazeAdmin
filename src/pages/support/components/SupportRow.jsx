...
const SupportRow = ({ displayData, index }) => {
    ...
    const handleAgentAssignment = (values) => {
    ...
    return (
        <>
            <tr className={`hover:bg-green-800 hover:cursor-pointer ${index % 2 === 0 ? "bg-green-950" : ""}`}>
                ...
                <td className='px-4 py-2'>
                    <div className='flex items-center gap-2'>
                        <div className={`w-4 h-4 rounded-sm ${displayData.priorityColor}`}></div>
                ...
                <td className='px-4 py-2 text-center'>
                    <button onClick={() => openAssignModal(displayData)} className="border border-green-600 hover:bg-green-600 text-white px-4 py-2 rounded-xl">
                        Open Chat
                    </button>
                </td>
            </tr>
            ...
            {showChat && <ChatApp ticket={selectedTicket} closeModal={()=>setShowChat(false)} assignedAgent={assignedAgent} />}
        </>
    );
};
