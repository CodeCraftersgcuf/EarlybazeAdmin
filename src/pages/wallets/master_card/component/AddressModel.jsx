...
const AddressModel = ({ onClose, datalist }) => {
    return (
        <Overlay>
            <div className='absolute top-10 right-10 rounded-md bg-theme-dark shadow-md shadow-gray-800 min-w-[300px] p-8'>
                ...
                <p className='my-4'>
                    These addresses are the individual address of each network
                    under Ethereum blockchain
                </p>
                <div className='flex flex-col gap-4'>
                    {
                        datalist.map((address, index) => (
                            <div key={index} className="bg-green-950 p-3 mb-2 rounded-md flex items-center justify-between">
                                ...
                                <div className='key max-w-sm text-nowrap overflow-hidden flex gap-8 items-center'>
                                    ...
                                    <div className='copy-btn'>
                                        <button onClick={() => navigator.clipboard.writeText(address.key)} className="text-2xl">
                                            <i className="bi bi-copy"></i>
                                        </button>
                                    </div>
                                </div>
                                {/* <p className="text-white">{JSON.stringify(address)}</p> */}
                            </div>
                        ))
                    }
                </div>
